...
const dipendenti = [
  ...
  function createCard(dipendenti) {
    return `
      <div class="col-md-4">
        <div class="card mb-4 my-bg">
          <div class="card-body">
            <h5 class="text-center mb-3">${dipendenti.nome} ${dipendenti.cognome}</h5>
            <p class="card-text">Età: ${dipendenti.età}</p>
            <p class="card-text">Professione: ${dipendenti.professione}</p>
          </div>
        </div>
      </div>
    `;
  ...
  dipendenti.forEach(dipendenti => {
    container.innerHTML += createCard(dipendenti);
  });
